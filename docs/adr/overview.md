...
Prompt skill 通过 `activate_skill(name)` 激活。Phase 1 在 session 启动时、数据库和 session/artifact root 就绪之后，阻塞式冻结 `SKILL.md` 和 `references/**` 文件级快照与 hash，并在执行时把 active skill refs 作为结构化 runtime state 管理。skill registry snapshot 独立于 `sessions.config_snapshot_json` 持久化，并与 session/run 关联。active `SKILL.md` 内容不写入普通 conversation history，也不靠 `/compress` summary 恢复。

每次 model call 前，`PromptComposer` 根据 active skill refs 从冻结快照生成 runtime-supplied active `SKILL.md` context。该 context 在当次 `ModelContextFrame` 中对模型可见，并标记为本 turn 的 runtime authoritative context。`references/**` 不自动注入；模型需要时通过 brokered `load_skill_ref_file(skill_name, path)` 读取已激活 skill 的 frozen reference 文件，读取结果作为普通 durable conversation observation，可被后续 omission/compression 自然清理。workflow skill 由 orchestrator 在 Phase 3 直接路由，不通过 `activate_skill`。
...
- prompt 组合包含 stable system prompt、context summary、retained messages、runtime-supplied active skill context 和当前输入。
...
- automatic compression 保留当前 turn 的 protected suffix：当前用户输入、当前 turn 的 tool-call 消息、fresh tool result 和 follow-up tool-loop messages 不进入压缩调用，真实模型调用时原样追加。
- 压缩会替换 `ReplRuntime.conversation` 的可压缩部分，并重建当前或下一次 `ModelContextFrame`。
- 上下文压缩分层执行：大输出 artifact 化、旧 tool result 省略、conversation continuity summary。
- conversation compression 不是简单提炼内容，必须保留任务目标、已完成节点、文件、剩余计划、关键决定与约束，保证长 runtime 连续性。
- workflow state 不靠 summary 恢复。
- active skill refs、artifact ids、approval records、policy facts 由 runtime 结构化维护，不靠模型 summary 恢复。
