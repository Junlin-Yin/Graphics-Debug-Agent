...
policy, runtime-control constraints, and reusable session approval grants into
runtime-owned `PermissionRule` values. `ToolBroker` evaluates these rules through
`PermissionEvaluator`; raw TOML policy structures are declarations, not the
runtime decision format.

`PermissionRule.effect` supports `deny`, `allow`, `trust`, and `ask`. `trust` is
not a final allow: trusted paths influence approval-mode automation but do not
grant read, write, execute, or runtime-control permission by themselves.
...
Approval grants are session-local. Decisions are persisted for audit, but
grants do not apply to future sessions.
...
paths, and `activate_skill` includes skill name plus content hash.
...
write audit records. Policy denial, schema validation failure, and config errors
cannot be overridden by any approval mode.
...
4. check allow and trust rules, including shell allowlist, path trust, and
...
- Path policy, shell policy, runtime-control decisions, and approval grants
  share one normalized permission rule model.
