...
支持 running turn 中断、idle session terminalization、active shell process best-effort termination，以及特定 terminalized long-lived prompt session 的 resume。

Phase 3 不做 `/cancel`、non-terminal session attach、one-shot resume、token-level resume、tool-mid-flight resume、subagent cancellation、PTY shell 或 long-running shell runtime。
...
具体新增 schema、事件、状态、tool result 字段和恢复语义必须在对应 phase 的 `scope.md` / specs / tests 中定义。
