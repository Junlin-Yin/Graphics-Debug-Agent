...
- Replace the legacy Phase 0 default system prompt with a phase-neutral generic `debug-agent` harness prompt, while keeping RenderDoc procedure in user prompts and prompt skills.
- Make active skill resource loading requirements clearer in model-visible active skill context and the `load_skill_resource` tool description.
...
- No broad prompt framework redesign beyond replacing the built-in default system prompt constant and clarifying active skill resource affordances.
...
- `docs/phase-4/`: implementation plan, tests, scope, architecture, and manual smoke evidence records. Contract/spec changes require human approval.
...
- [x] If package metadata changes were required, run `uv lock` and verify the lockfile change belongs to that dependency/metadata change. Not applicable; package metadata changes were not required.
- [x] Record package smoke command output summary in the implementation review evidence.
- [x] Record external adapted skill location/version or content hash.
- [x] Record how the adapted skill was exposed under project skill discovery.
- [x] Record evidence that `renderdoc-gpu-debug` was discoverable and activated.
- [x] Record session id, run id, command sequence, brokered `shell_exec` observations, brokered `view_image` result when an image is produced, trace path, metrics path, observed result, and known limitations.
- [x] Record Windows version and runner/machine details.
- [x] Record RenderDoc and `rdc` versions.
- [x] Record relevant environment variables and PATH notes.
- [x] Record sample `.rdc` path, real `rdc` command sequence, expected result, observed result, session id, run id, trace path, metrics path, and known limitations.
- [x] If the same run satisfies adapted skill smoke and Windows + real `rdc` smoke, ensure all fields required by both records are present.
- [x] Confirm fake `rdc` readiness runs in ordinary integration automation.
- [x] Confirm manual adapted `renderdoc-gpu-debug` skill smoke record exists.
- [x] Confirm Windows + real `rdc` smoke record exists for v1 completion.
...
### Milestone 7: Main-Agent Harness Prompt, Skill Resource Affordance, And View Image Query Correction

**Objective:** replace the legacy Phase 0 default prompt naming/text with the Phase 4 generic runtime harness prompt, make active skill resource loading requirements more visible to the model during real skill-driven debugging runs, and strengthen the default `view_image` query for generic visual debugging observations.
...
external prompt drafts, scratch files, historical phase docs, or source-code
comments to reconstruct the target prompt or replacement text. The exact
