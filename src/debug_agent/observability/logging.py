...
def write_event_log(sessions_root: Path, event: RunEvent) -> None:
    EventsJsonlWriter(_log_path(sessions_root, event.session_id)).write(
...
def write_runtime_log(
    ...
    *,
    ...
    EventsJsonlWriter(_log_path(sessions_root, session_id)).write(
...
def _log_path(sessions_root: Path, session_id: str) -> Path:
    return Path(sessions_root).resolve() / session_id / "logs" / "events.jsonl"
